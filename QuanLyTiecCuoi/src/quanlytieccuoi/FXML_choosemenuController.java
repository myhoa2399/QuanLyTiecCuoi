...
public class FXML_choosemenuController implements Initializable {
    ...
    @FXML
    private RadioButton rdFood;
    @FXML
    private RadioButton rdDrink;
    @FXML
    private RadioButton rdService;
    @FXML
        ...
        if(this.tbOrder.getSelectionModel().getSelectedItem() != null){
            ...
            alert.showAndWait().ifPresent(rs -> {
                if (rs == ButtonType.OK) {
                    ...
                    int unitPrice = priceHandler(str); // giá của fds xóa * amount
                    tbOrder.getItems().remove(tbOrder.getSelectionModel().getSelectedItem());
                    int totalMonney = priceHandler(totalM.getText()); // lấy giá hiện tại của totalMoney
               ...
               int kq = checkAmount(s);
                if(kq == 0){ // hơp lệ
                    ...
                    if(x != 0){
                        loadOrder(f, x); 
                        //tinh tien
                        int unitPrice = priceHandler(f.getUnitPrice()); // this price * amount
                        int totalMoney = priceHandler(totalM.getText());
    ...
    public void saveOrder(){
        if(this.tbOrder.getItems().size() != 0){
            try {
                ...
                orders.setTotalMoney(String.format("%,d", priceHandler(totalM.getText())));
                ...
                for(FoodDrinkService fds : tbOrder.getItems()){
                    ...
                    int p= priceHandler(fds.getPrice());
